...
export const useEnhancedAICodeGeneration = () => {
  ...
  const generateCodeWithAI = useCallback(async (
    ...
    try {
      console.log('🤖 Starting enhanced AI code generation...');
      
      // Log chat history
      appendToChatHistory(userRequest, 'user');

      // Use the enhanced edge function with Together AI fallback
      const response = await supabase.functions.invoke('enhanced-professional-website', {
        body: {
          userRequest,
          channelData,
          projectContext: {
      ...
      toast({
        title: "🎯 Enhanced AI Changes Applied",
        description: data.codeDescription || "Your modifications have been implemented with professional quality.",
      });
