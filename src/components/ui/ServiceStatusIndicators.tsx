...
const ServiceStatusIndicators: React.FC = () => {
  ...
  const checkServiceStatus = async () => {
    ...
    try {
      ...
      const [youtubeKeys, openrouterKeys, githubKeys, netlifyKeys] = await Promise.all([supabase.from('youtube_api_keys').select('quota_used, quota_limit').eq('is_active', true), supabase.from('openrouter_api_keys').select('credits_used, credits_limit').eq('is_active', true), supabase.from('github_api_keys').select('rate_limit_used, rate_limit_limit').eq('is_active', true), supabase.from('netlify_api_keys').select('deployments_count, deployments_limit').eq('is_active', true)]);

      // Check recent usage from logs (last 2 minutes for real-time indication)
      const twoMinutesAgo = new Date(Date.now() - 2 * 60 * 1000).toISOString();
      const {
        data: recentUsage
      } = await supabase.from('api_usage_logs').select('provider').gte('created_at', twoMinutesAgo).eq('status', 'success');
      ...
      setStatus(newStatus);
      setStats(newStats);
      console.log('✅ Real-time service status updated:', {
        status: newStatus,
        stats: newStats
      });
    } catch (error) {
  ...
  return <TooltipProvider>
      <div className="flex items-center gap-2">
        ...
        return <Tooltip key={service.name}>
              ...
              <TooltipContent>
                <div className="space-y-1">
                  ...
                  {status[service.key] === 'available' && serviceStats.limit > 0 && <div className="space-y-1">
                      ...
                      <div className="w-24 h-1 bg-gray-600 rounded-full overflow-hidden">
                        <div className={`h-full transition-all duration-300 ${usagePercentage > 80 ? 'bg-red-400' : usagePercentage > 60 ? 'bg-yellow-400' : 'bg-green-400'}`} style={{
